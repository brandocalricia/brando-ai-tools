...
async function checkExistingSession() {
  ...
  try {
    ...
    isPro = data.pro_extensions?.jobs || data.plan === "pro";
    usageToday = data.usage?.jobs || 0;
...
async function loadCapturedJob() {
  ...
  if (stored.jobContext && stored.jobContext.text) {
    const age = Date.now() - stored.jobContext.timestamp;
    if (age < 5 * 60 * 1000) {
      const analyzeTA = document.getElementById("job-text");
      if (analyzeTA && !analyzeTA.value) {
        analyzeTA.value = stored.jobContext.text;
      }
    }
    await chrome.storage.local.remove(["jobContext"]);
  }
}
...
async function handleLogin() {
  ...
  try {
    ...
    isPro = data.user.plan === "pro" || (data.user.plan && data.user.plan.includes("jobs"));
    usageToday = 0;
...
function setupButtons() {
  ...
  document.getElementById("upgrade-btn")?.addEventListener("click", () => openUpgrade("jobs"));
  document.getElementById("cover-upgrade-btn")?.addEventListener("click", () => openUpgrade("jobs"));
...
async function runAnalyze() {
  ...
  if (!jobText) {
    showError("Paste a job listing first.");
    return;
  }
  if (!canGenerate()) {
    showUpgradePrompt();
  ...
  lastRequest = { mode: "analyze", job_text: jobText };
  await callAPI(lastRequest);
}

async function runMatch() {
  const resumeText = document.getElementById("resume-text").value.trim();
  const jobText = document.getElementById("match-job-text").value.trim();
  if (!resumeText) {
    showError("Paste your resume first.");
    return;
  }
  if (!jobText) {
    showError("Paste the job listing.");
    return;
  }
  if (!canGenerate()) {
    showUpgradePrompt();
  ...
  lastRequest = { mode: "match", resume_text: resumeText, job_text: jobText };
  await callAPI(lastRequest);
}

async function regenerate() {
  if (lastRequest) await callAPI(lastRequest);
}
...
function setButtonsDisabled(disabled) {
  ...
  document.getElementById("match-btn").disabled = disabled;
}
...
async function openUpgrade(extension = "jobs") {
  try {
